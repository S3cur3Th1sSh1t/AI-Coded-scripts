...
def convert_to_bloodhound_ce(data, domain, output_dir):
    ...
    try:
        ...
        if bloodhound_ce:
            ...
            # Create subdirectory for BloodHound files
            if base_name:
                bh_dir = output_path / f"{base_name}_bhce_{timestamp}"
            else:
                bh_dir = output_path / f"bhce_{domain}_{timestamp}"
            ...
            print(f"\n[+] BloodHound CE files saved to: {bh_dir}/")
            for filename in files_created:
                print(f"    - {filename}")
            print(f"\n[*] Import entire directory to BloodHound CE")
            print(f"[*] Or zip and upload: cd {bh_dir} && zip -r ../bloodhound_{timestamp}.zip *.json")
            
        else:
            # Save single JSON for BOFHound
            if base_name:
                json_filename = output_path / f"{base_name}_{timestamp}.json"
            else:
                json_filename = output_path / f"ldap_enum_{domain}_{timestamp}.json"
...
def convert_to_bloodhound_ce(data, domain):
    """
    Convert our data format to BloodHound CE compatible JSON
    
    BloodHound CE expects separate files for each object type:
    - computers.json
    - users.json
    - groups.json
    - domains.json
    ...
    Each with specific field names and structure
    """
    bloodhound_data = {
        "data": [],
        "meta": {
    ...
    # Build domain object
    domain_obj = {
        "ObjectIdentifier": data['meta'].get('domain', domain).upper(),
        "Properties": {},
        "Trusts": [],
        "Links": [],
        "ChildObjects": [],
    ...
    if 'domain_info' in data.get('data', {}):
        info = data['data']['domain_info']
        domain_obj['Properties'] = {
            "domain": domain.upper(),
            "name": domain.upper() + "@" + domain.upper(),
            "domainsid": info.get('objectSid', ''),
            "functionallevel": info.get('functional_level', ''),
            "machineaccountquota": info.get('machine_account_quota', 10)
        }
    
    # Add trusts (CE requires string values, not integers)
    if 'trusts' in data.get('data', {}):
        for trust in data['data']['trusts']:
            trust_obj = {
                "TargetDomainSid": trust.get('objectSid', ''),
                "TargetDomainName": trust.get('trustPartner', ''),
                "IsTransitive": True,  # Most trusts are transitive
                "SidFilteringEnabled": True,
                "TrustDirection": trust.get('trustDirection', 'Unknown'),  # Already string
                "TrustType": trust.get('trustType', 'Unknown')  # Already string
            }
            domain_obj['Trusts'].append(trust_obj)
    ...
    if 'gpos' in data.get('data', {}):
        for gpo in data['data']['gpos']:
            link_obj = {
                "IsEnforced": False,
                "GUID": gpo.get('name', '').strip('{}')
            }
            domain_obj['Links'].append(link_obj)
    
    bloodhound_data['data'].append(domain_obj)
    
    # Note: Full BloodHound CE support would require:
    # - Separate JSON files for users, computers, groups, etc.
    # - ACL parsing and conversion to BloodHound ACE format
    # - Group membership resolution
    # - Local admin rights collection
    # - Session data collection
    # This is a simplified domain-level export
    
    bloodhound_data['meta']['count'] = 1
